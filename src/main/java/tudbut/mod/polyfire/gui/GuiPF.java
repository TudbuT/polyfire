package tudbut.mod.polyfire.gui;

import net.minecraft.client.gui.GuiButton;
import net.minecraft.client.gui.GuiScreen;
import org.lwjgl.input.Mouse;
import tudbut.mod.polyfire.PolyFire;
import tudbut.mod.polyfire.mods.ClickGUI;
...
public class GuiPF extends GuiScreen {
    ...
    public void resetButtons() {
        ...
        Button[] buttons = new Button[PolyFire.modules.length];
        for (int i = 0, j = 0; i < PolyFire.modules.length; i++) {
            int x = j / 15;
            int y = j - x * 15;
            
            // Don't add the button if it isn't requested
            if (!PolyFire.modules[i].displayOnClickGUI())
                continue;
            
            // Create the button
            int r = i;
            Button b = new Button(
                    10 + (155 * x), 10 + (y * 25), PolyFire.modules[r].toString() + ": " + PolyFire.modules[r].enabled,
                    (text) -> {
                        if (PolyFire.modules[r].enabled = !PolyFire.modules[r].enabled)
                            PolyFire.modules[r].onEnable();
                        else
                            PolyFire.modules[r].onDisable();
    ...
    public static class Button {
        ...
        protected void onTick(GuiPF gui) {
            this.color = ClickGUI.getInstance().getTheme().getButtonColor();
            if (module != null) {
